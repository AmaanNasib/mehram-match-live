...
const TotalShortlistAgent = () => {
  ...
  const applyFilters = (updatedFilters) => {
    ...
    setFilteredItems(
      matchDetails?.filter((match) => {
        return (
          (updatedFilters.id ? match?.action_on?.id == updatedFilters.id : true) &&
          (updatedFilters.name ? match?.action_on?.name?.toLowerCase().includes(updatedFilters.name.toLowerCase()) : true) &&
          (updatedFilters.city ? match?.action_on?.city?.toLowerCase().includes(updatedFilters.city.toLowerCase()) : true) &&
          (updatedFilters.startDate && updatedFilters.endDate
            ...
            : true) &&
          (updatedFilters.sectSchoolInfo ? match?.action_on?.sect_school_info?.toLowerCase().includes(updatedFilters.sectSchoolInfo.toLowerCase()) : true) &&
          (updatedFilters.profession ? match?.action_on?.profession?.toLowerCase().includes(updatedFilters.profession.toLowerCase()) : true) &&
          (updatedFilters.status ? match?.status?.toLowerCase().includes(updatedFilters.status.toLowerCase()) : true) &&
          (updatedFilters.martialStatus ? match?.action_on?.martial_status?.toLowerCase().includes(updatedFilters.martialStatus.toLowerCase()) : true)
        );
      })
    );
  };
  const distinctIds = [...new Set(matchDetails?.map((match) => match?.action_on?.id))];
  const distinctNames = [...new Set(matchDetails?.map((match) => match?.action_on?.name))];
  const distinctCities = [...new Set(matchDetails?.map((match) => match?.action_on?.city))];
  const distinctDobs = [...new Set(matchDetails?.map((match) => match?.action_on?.date))];
  const distinctSchoolInfo = [...new Set(matchDetails?.map((match) => match?.action_on?.sect_school_info))];
  const distinctProfessions = [...new Set(matchDetails?.map((match) => match?.action_on?.profession))];
  const distinctStatuses = [...new Set(matchDetails?.map((match) => match?.status))];
  const distinctMaritalStatuses = [...new Set(matchDetails?.map((match) => match?.action_on?.martial_status))];
  useEffect(() => {
  ...
  const handleDelete = (id) => {
    setMatchDetails({ shortlisted_users: matchDetails?.filter(match => match?.action_on?.id !== id) });
  };
        ...
        <table className="interest-table">
          ...
          <tbody>
            {currentItems.map((match) => (
              <tr key={match?.action_on?.id} style={{ cursor: "pointer" }}>
                <td>{match?.action_on?.id}</td>
                <td>{match?.action_on?.name || 'NA'}</td>
                <td>{match?.action_on?.city || 'NA'}</td>
                <td>{match?.date	 || 'NA'}</td>
                <td>{match?.action_on?.sect_school_info || 'NA'}</td>
                <td>{match?.action_on?.profession || 'NA'}</td>
                <td>
                  <span className={`marital-badge ${match?.action_on?.martial_status ? match?.action_on?.martial_status.toLowerCase().replace(" ", "-") : 'not-mentioned'}`}>
                    {match?.action_on?.martial_status || 'NA'}
                  </span>
                </td>
                <td>
                  <AiOutlineDelete
                    ...
                    onClick={(e) => {
                      ...
                      handleDelete(match?.action_on?.id);
                    }}
                  />
                </td>
              </tr>
