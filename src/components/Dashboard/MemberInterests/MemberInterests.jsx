...
const MemberInterests = () => {
  ...
  useEffect(() => {
    const getAgent = async () => {
      const agentresponce = await api.get(
        `${process.env.REACT_APP_API_URL}/api/agent/`
      );
      const result = agentresponce.data;
        ...
        setterFunction: (data) => {
          ...
          if (data.success) {
            ...
            const sentInterests = interests.filter(
              (interest) => interest.type === "sent"
            );
            const receivedInterests = interests.filter(
              (interest) => interest.type === "received"
            );
  ...
  const handleWithdrawInterest = async (interestId, targetId) => {
    try {
      setWithdrawingInterest(interestId);
      // const payload = {
      //     member_id: interestId,
      //     // agent_id: agentId,
      //     target_user_id: targetId,
      //   }

      //   const responce = await api.post('/api/agent/member/withdraw-intrest/', payload);
      //   const result = responce.data

      //   console.log("withdraw api", result)
      const parameter = {
        url: `/api/agent/member/withdraw-interest/`,
        payload: {
        ...
        <div className="member-interests-container">
          <div className="member-interests-error-container">
            ...
            <button
              ...
              className="member-interests-back-btn"
            >
              Go Back
            </button>
          </div>
        ...
        {memberData && (
          <div className="member-info-card">
            <div className="member-avatar">
              <img
                src={
                  memberData.profile_photo?.upload_photo
            ...
            <div className="member-details">
              ...
              <p className="member-id">
                Member ID: {memberData.id || memberData.member_id || "N/A"}
              </p>
          ...
          <div className="member-interests-stats-content">
            <div className="member-interests-stats-row">
              <div className="member-interests-stat-item">
                <div className="member-interests-stat-label">
                  Total Interests
                </div>
                <div className="member-interests-stat-value total">
                  {interestStats.total}
                </div>
              </div>
              <div className="member-interests-stat-item">
                ...
                <div className="member-interests-stat-value sent">
                  {interestStats.sent}
                </div>
              </div>
              <div className="member-interests-stat-item">
                ...
                <div className="member-interests-stat-value received">
                  {interestStats.received}
                </div>
              </div>
            </div>
              ...
              <div className="member-interests-breakdown-column">
                ...
                <div className="member-interests-breakdown-items">
                  <span className="member-interests-breakdown-item pending">
                    <span className="member-interests-breakdown-label">
                      Pending:
                    </span>
                    <span className="member-interests-breakdown-count">
                      {
                        sentInterests.filter((i) => i.status === "Pending")
                          .length
                      }
                    </span>
                  </span>
                  ...
                  <span className="member-interests-breakdown-item rejected">
                    ...
                    <span className="member-interests-breakdown-count">
                      {
                        sentInterests.filter((i) => i.status === "Rejected")
                          .length
                      }
                    </span>
                  </span>
              ...
              <div className="member-interests-breakdown-column">
                ...
                <div className="member-interests-breakdown-items">
                  ...
                  <span className="member-interests-breakdown-item rejected">
                    ...
                    <span className="member-interests-breakdown-count">
                      {
                        receivedInterests.filter((i) => i.status === "Rejected")
                          .length
                      }
                    </span>
                  </span>
                  ...
                  <tbody>
                    {sentInterests.map((interest, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <span
                            className={`member-interests-table-status-badge member-interests-table-status-${
                  ...
                  <tbody>
                    {receivedInterests.map((interest, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {interest.from_user?.id ||
                            interest.from_user?.member_id ||
                            "N/A"}
                        </td>
                        <td>
                          {interest.date
                          ...
                          <div className="member-interests-action-buttons">
                            {interest.status === "Pending" ? (
                              <>
                                <button
                                  onClick={() =>
                                    handleAcceptInterest(interest.id)
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleRejectInterest(interest.id)
                                  }
                                  disabled={processingInterest === interest.id}
