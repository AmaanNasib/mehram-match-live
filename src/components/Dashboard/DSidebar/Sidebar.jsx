...
const Sidebar = ({setApiData, onClose, reloadOriginalData}) => {
  ...
  // Debounced live search by Member ID - Word by word search
  const debounceRef = useRef(null);
  useEffect(() => {
    if (!formData.memberID || formData.memberID.trim().length === 0) return; // only when user types something
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      const parameter = {
        url: '/api/user/filter/',
        payload: {
          // Trim member ID and enable partial match for word-by-word search
          member_id: formData.memberID.trim(),
          search_mode: 'partial', // Enable partial matching for member ID
        ...
        setUserId: (data) => {
          console.log('Member ID search response:', data);
  ...
  const handleSearch = () => {
    ...
    console.log('Search button clicked with filters:', {
      ...
      sect_school_info: formData.sect,
